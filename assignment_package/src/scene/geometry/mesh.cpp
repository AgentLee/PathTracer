...
Bounds3f Triangle::WorldBound() const
{
    ...
    Point3f min(std::min(p1.x, std::min(p2.x, p3.x)),
                std::min(p1.y, std::min(p2.y, p3.y)),
                std::min(p1.z, std::min(p2.z, p3.z)));

    Point3f max(std::max(p1.x, std::max(p2.x, p3.x)),
                std::max(p1.y, std::max(p2.y, p3.y)),
                std::max(p1.z, std::max(p2.z, p3.z)));

    Bounds3f bounds = Union(p1, p2);
    bounds = Union(bounds, p3);

    return bounds.Apply(transform);
}
...
void Triangle::ComputeTriangleTBN(const Point3f &P, Normal3f *nor, Vector3f *tan, Vector3f *bit, const Point2f &uv) const
{
    *nor = GetNormal(P);
    ...
    // Following opengl-tutorial method on how to compute tangent and bitangent with UV coordinates
//    Point3f p1 = points[0];
//    Point3f p2 = points[1];
//    Point3f p3 = points[2];
...
//    // Triangle Edges delta
//    Point3f deltaPos1 = p2-p1;
//    Point3f deltaPos2 = p3-p1;

//    // UV delta
//    Point2f deltaUV1 = uv2-uv1;
//    Point2f deltaUV2 = uv3-uv1;

//    float r = 1.0f / (deltaUV1.x * deltaUV2.y - deltaUV1.y * deltaUV2.x);

//    if ((deltaUV1.x * deltaUV2.y - deltaUV1.y * deltaUV2.x) == 0.f) {
//        *tan = glm::normalize(p2 - p1);
