...
bool Scene::Intersect(const Ray &ray, Intersection *isect) const
{
    ...
    else
    {
        ...
        if(makeCSG) {
            Intersection csgIsect;
            bool csgResult = csg->Intersect(csg->root, ray, &csgIsect);
        ...
        for(std::shared_ptr<Primitive> p : primitives)
        {
            ...
            if(p->Intersect(ray, &testIsect))
            {
                ...
                {
                    if(makeCSG) {
                        if(p == csg->root->left->prim || p == csg->root->right->prim) {
                            if(csg->root->operation > 1) {
                                continue;
                            }
                        }
                    }
