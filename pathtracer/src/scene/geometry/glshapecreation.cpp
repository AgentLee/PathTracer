...
{
    GLuint cub1_idx[CUB_IDX_COUNT];
    glm::vec3 cub1_vert_pos[CUB1_VERT_COUNT];
    glm::vec3 cub1_vert_nor[CUB1_VERT_COUNT];
    glm::vec3 cub1_vert_col[CUB1_VERT_COUNT];

    createCubeVertexPositions(cub1_vert_pos);
    createCubeVertexNormals(cub1_vert_nor);
    createCubeIndices(cub1_idx);


    Color3f color(colorRNG.nextFloat(), colorRNG.nextFloat(), colorRNG.nextFloat());

    for(int i = 0; i < CUB1_VERT_COUNT; i++){
        cub1_vert_col[i] = color;
    }
    ...
    bufIdx.bind();
    bufIdx.setUsagePattern(QOpenGLBuffer::StaticDraw);
    bufIdx.allocate(cub1_idx, CUB1_IDX_COUNT * sizeof(GLuint));

    bufPos.create();
    bufPos.bind();
    bufPos.setUsagePattern(QOpenGLBuffer::StaticDraw);
    bufPos.allocate(cub1_vert_pos,CUB1_VERT_COUNT * sizeof(glm::vec3));

    bufNor.create();
    bufNor.bind();
    bufNor.setUsagePattern(QOpenGLBuffer::StaticDraw);
    bufNor.allocate(cub1_vert_nor, CUB1_VERT_COUNT * sizeof(glm::vec3));

    bufCol.create();
    bufCol.bind();
    bufCol.setUsagePattern(QOpenGLBuffer::StaticDraw);
    bufCol.allocate(cub1_vert_col, CUB1_VERT_COUNT * sizeof(glm::vec3));
}
