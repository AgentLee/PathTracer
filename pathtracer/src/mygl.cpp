...
#include <integrators/photonmapping.h>
...
void MyGL::RenderScene()
{
    ...
    // Build CSG Tree
    // Assume that's what the user wants
    // TODO: Add user option
    if(scene.makeCSG && !scene.csg) {
        scene.csg = new CSG(scene.primitives.toVector().toStdVector());
    }

    if(makeKDT && !scene.kdtree) {
        scene.kdtree = new KDTreeAccel(scene.primitives.toVector().toStdVector(), 80, 1, 0.5f, maxBVHPrims, -1);
    }
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case VOLUMETRIC:
                ...
                break;
            case PHOTON_MAPPING:
                rt = new PhotonMapping(tileBounds, &scene, sampler->Clone(seed), recursionLimit);
                break;
            }
